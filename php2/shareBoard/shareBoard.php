...
<?php
    ...
    $sql = "SELECT b.boardTitle, b.boardContents, m.youName, b.regTime, b.boardView FROM board b JOIN members m ON(b.memberID = m.memberID) ORDER BY boardID DESC LIMIT {$viewLimit}, {$viewNum}";
    $result = $connect -> query($sql);
    ...
    if($result){
        ...
        if($count > 0){
            for($i=1; $i<$count; $i++){
                ...
                echo "<div class='list__name'> <b>".$info['youName']."</b><small>".date('Y-m-d', $info['regTime'])."</small></div>";
